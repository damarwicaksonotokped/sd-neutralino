(function () {
	...
	class Interface {
		...
		async _downloadPNGClick(e) {
			this.forceRender();
			this.diagram.getPNGBlob({ resolution: PNG_RESOLUTION })
				.then(({ blob, latest }) => {
					console.log(blob, latest);
					if (latest) {
						this.pngBlob = blob;
					}
				});
			this._hideURLBuilder();
			this.entry = await saveDialog('Save to File', {
				filters: [
					{ name: 'Images', extensions: ['svg', 'png'] },
				]
			});
			this.pngBlob.arrayBuffer()
				.then((data) => {
					writeBinaryFile(this.entry, data)
						.then(function (res) {
		...
		async _downloadSVGClick() {
			this.forceRender();
			const data = this.diagram.getSVGCodeSynchronous();
			this._hideURLBuilder();
			this.entry = await saveDialog('Save to File', {
				filters: [
					{ name: 'Images', extensions: ['svg', 'png'] },
			...
			await writeFile(this.entry, data)
				.then(function (resolved) {
					console.log("image saved ", this.entry)
				}).catch(function (err) {
					console.log(err)
				})
		}
